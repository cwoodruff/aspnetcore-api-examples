...
[ApiVersion("1.0")]
public class PlaylistController(IChinookSupervisor chinookSupervisor, ILogger<PlaylistController> logger)
    : ControllerBase
{
    [HttpGet]
        ...
        [FromQuery] int pageSize)
    {
        try
        {
            var playlists = await chinookSupervisor.GetAllPlaylist(pageNumber, pageSize);
        ...
        {
            logger.LogError($"Something went wrong inside the PlaylistController Get action: {ex}");
    ...
    public async Task<ActionResult<PlaylistApiModel>> Get(int id)
    {
        try
        {
            var playlist = await chinookSupervisor.GetPlaylistById(id);
        ...
        {
            logger.LogError($"Something went wrong inside the PlaylistController GetById action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
    ...
    public async Task<ActionResult<PlaylistApiModel>> Post([FromBody] PlaylistApiModel input)
    {
        try
        {
            ...
            {
                return Ok(await chinookSupervisor.AddPlaylist(input));
            }
        }
        catch (ValidationException ex)
        {
            logger.LogError($"Something went wrong inside the PlaylistController Add Playlist action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
    ...
    public async Task<ActionResult<PlaylistApiModel>> Put(int id, [FromBody] PlaylistApiModel input)
    {
        try
        {
            ...
            {
                return Ok(await chinookSupervisor.UpdatePlaylist(input));
            }
        }
        catch (ValidationException ex)
        {
    ...
    public async Task<ActionResult> Delete(int id)
    {
        try
        {
            return Ok(await chinookSupervisor.DeletePlaylist(id));
        }
        catch (Exception ex)
        {
            logger.LogError($"Something went wrong inside the PlaylistController GetById action: {ex}");
