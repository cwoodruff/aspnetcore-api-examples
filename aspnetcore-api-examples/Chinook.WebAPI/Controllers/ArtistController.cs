...
[ApiVersion("1.0")]
public class ArtistController(IChinookSupervisor chinookSupervisor, ILogger<ArtistController> logger)
    : ControllerBase
{
    [HttpGet]
    ...
    public async Task<ActionResult<PagedList<ArtistApiModel>>> Get([FromQuery] int pageNumber, [FromQuery] int pageSize)
    {
        try
        {
            var artists = await chinookSupervisor.GetAllArtist(pageNumber, pageSize);
    ...
    public async Task<ActionResult<ArtistApiModel>> Get(int id)
    {
        try
        {
            var artist = await chinookSupervisor.GetArtistById(id);
        ...
        {
            logger.LogError($"Something went wrong inside the ArtistController GetById action: {ex}");
    ...
    public async Task<ActionResult<ArtistApiModel>> Post([FromBody] ArtistApiModel input)
    {
        try
        {
            ...
            return Ok(await chinookSupervisor.AddArtist(input));
        }
        catch (ValidationException ex)
        {
            logger.LogError($"Something went wrong inside the ArtistController Add Artist action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
    ...
    public async Task<ActionResult<ArtistApiModel>> Put(int id, [FromBody] ArtistApiModel input)
    {
        try
        {
            ...
            return Ok(await chinookSupervisor.UpdateArtist(input));
        }
        catch (ValidationException ex)
        {
            logger.LogError($"Something went wrong inside the ArtistController Update Artist action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
    ...
    public async Task<ActionResult> Delete(int id)
    {
        try
        {
            return Ok(await chinookSupervisor.DeleteArtist(id));
        }
        catch (Exception ex)
        {
            logger.LogError($"Something went wrong inside the ArtistController Delete action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
