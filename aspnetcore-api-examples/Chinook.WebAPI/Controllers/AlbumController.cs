    ...
    : ControllerBase
{
    ...
    public async Task<ActionResult<PagedList<AlbumApiModel>>> Get([FromQuery] int pageNumber, [FromQuery] int pageSize)
    {
        try
        {
            var albums = await chinookSupervisor.GetAllAlbum(pageNumber, pageSize);
        ...
        catch (Exception ex)
        {
            ...
            logger.LogError($"Something went wrong inside the AlbumController Get All Album action: {ex}");
            return new ObjectResult(problemDetails)
            {
                ContentTypes = { "application/problem+json" },
                StatusCode = 403,
    ...
    public async Task<ActionResult<AlbumApiModel>> Get(int id)
    {
        try
        {
            var album = await chinookSupervisor.GetAlbumById(id);
    ...
    public async Task<ActionResult<AlbumApiModel>> Post([FromBody] AlbumApiModel input)
    {
        try
        {
            ...
            return Ok(await chinookSupervisor.AddAlbum(input));
        }
    ...
    public async Task<ActionResult<AlbumApiModel>> Put(int id, [FromBody] AlbumApiModel input)
    {
        try
        {
            ...
            return Ok(await chinookSupervisor.UpdateAlbum(input));
        }
        catch (ValidationException ex)
        {
            var problemDetails = new AlbumProblemDetails
    ...
    public async Task<ActionResult> Delete(int id)
    {
        try
        {
            return Ok(await chinookSupervisor.DeleteAlbum(id));
        }
        catch (Exception ex)
        {
            var problemDetails = new AlbumProblemDetails
        ...
        [FromQuery] int pageSize)
    {
        try
        {
            var albums = await chinookSupervisor.GetAlbumByArtistId(id, pageNumber, pageSize);
