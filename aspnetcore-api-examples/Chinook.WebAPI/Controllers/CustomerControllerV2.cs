...
[ApiVersion("2.0")]
public partial class CustomerController(IChinookSupervisor chinookSupervisor, ILogger<CustomerController> logger)
    : ControllerBase
{
    [HttpGet]
        ...
        [FromQuery] int pageSize)
    {
        try
        {
            var customers = await chinookSupervisor.GetAllCustomer(pageNumber, pageSize);
        ...
        {
            logger.LogError($"Something went wrong inside the CustomerController Get action: {ex}");
    ...
    public async Task<ActionResult<CustomerApiModel>> Get(int id)
    {
        try
        {
            var customer = await chinookSupervisor.GetCustomerById(id);
        ...
        {
            logger.LogError($"Something went wrong inside the CustomerController GetById action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
    ...
    public async Task<ActionResult<CustomerApiModel>> Post([FromBody] CustomerApiModel input)
    {
        try
        {
            ...
            return Ok(await chinookSupervisor.AddCustomer(input));
        }
        catch (ValidationException ex)
        {
            logger.LogError($"Something went wrong inside the CustomerController Add Customer action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
    ...
    public async Task<ActionResult<CustomerApiModel>> Put(int id, [FromBody] CustomerApiModel input)
    {
        try
        {
            ...
            return Ok(await chinookSupervisor.UpdateCustomer(input));
        }
        catch (ValidationException ex)
        {
            logger.LogError($"Something went wrong inside the CustomerController Update Customer action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
    ...
    public async Task<ActionResult> Delete(int id)
    {
        try
        {
            return Ok(await chinookSupervisor.DeleteCustomer(id));
        }
        catch (Exception ex)
        {
            logger.LogError($"Something went wrong inside the CustomerController Delete action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
    ...
    public async Task<ActionResult<EmployeeApiModel>> GetBySupportRepId(int id)
    {
        ...
        {
            logger.LogError($"Something went wrong inside the CustomerController GetBySupportRepId action: {ex}");
            return StatusCode((int)HttpStatusCode.InternalServerError,
